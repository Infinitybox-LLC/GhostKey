...
const byte masterRfidKey[5] = {67, 0, 25, 249, 64};   // 43:00:19:F9:40
...
static bool isMasterRfidTag(byte* tag) {
  return compareTagData((byte*)masterRfidKey, tag) || compareTagData((byte*)masterRfidKey2, tag);
}
...
bool addRfidKey(byte *newKey) {
    ...
    // Prevent adding a master key to user storage
    if (isMasterRfidTag(newKey)) {
...
void setup() {
    ...
    if (wakeup_reason == ESP_SLEEP_WAKEUP_EXT0 && wasInDeepSleep) {
        // Woke from hardware deep sleep via start button (BUTTON_PIN / GPIO7 on REV2)
...
void loop() {
    ...
    // Handle RFID config mode hold logic (Ghost Power only mode OR before setup complete)
    if ((!ghostKeyEnabled && ghostPowerEnabled && currentState != CONFIG_MODE) || 
        (!firstSetupComplete && currentState != CONFIG_MODE)) {
...
void controlRelays() {
    ...
    if (engineRunning) {
        ...
        // Starting sequence - IGN1, IGN2 and START (IGN2 & accessory off)
        newAccessory = LOW; newIgnition1 = HIGH; newIgnition2 = LOW; newStart = HIGH;
    } else {
    ...
    switch(newState) {
        ...
        case POWER_DEEP_SLEEP:
            ...
            if (useHardwareDeepSleep) {
                ...
                // Configure wake-up source: start button on LOW signal
