# 1 "/var/folders/bj/9hlf5cr55td9xzwfkbc27rdr0000gy/T/tmp_zntvhi3"
#include <Arduino.h>
# 1 "/Users/zkhan/GhostKey/src/GhostKey.ino"
...
#define BRAKE_PIN 18
#define ACCESSORY_INPUT_PIN 19


#define LED_PIN 23
#define BUTTON_LED_PIN 26
#define BEEPER_PIN 25


#define RFID_DEMOD_OUT 12
#define RFID_SHD 13
#define RFID_MOD 27
#define RFID_RDY_CLK 14


#define RELAY_ACCESSORY 15
#define RELAY_IGNITION1 5
#define RELAY_IGNITION2 4
#define RELAY_START 16
#define RELAY_SECURITY 17
...
#define RFID_DELAYVAL 384
#define RFID_TIMEOUT 2000
#define MAX_RFID_KEYS 5
#define RFID_AUTH_TIMEOUT 30000
...
#define RSSI_MAX_VALID -10
#define RSSI_INVALID_VALUE -99


#define RSSI_SHORT_TERM_SIZE 30
#define RSSI_MEDIUM_TERM_SIZE 50
#define RSSI_LONG_TERM_SIZE 300
#define CONFIDENCE_AUTH_THRESHOLD 65.0f
#define CONFIDENCE_DEAUTH_THRESHOLD 50.0f
#define STABILITY_WEIGHT 35.0f
#define TREND_WEIGHT 25.0f
#define STRENGTH_WEIGHT 40.0f
#define MIN_READINGS_FOR_ANALYSIS 3
#define TREND_WINDOW_MS 8000
#define STABILITY_THRESHOLD 12.0f
#define STRONG_SIGNAL_THRESHOLD -60
#define WEAK_SIGNAL_THRESHOLD -80
#define VERY_STRONG_SIGNAL_THRESHOLD -50
#define PROXIMITY_BONUS_THRESHOLD -55


#define STATIONARY_BONUS_POINTS 10.0f
#define CONFIDENCE_MOMENTUM_RATE 0.25f
#define CONFIDENCE_MOMENTUM_RATE_FAST 0.40f
#define CONFIDENCE_CHANGE_THRESHOLD 20.0f
#define STATIONARY_MIN_SAMPLES 8
#define STATIONARY_MAX_STDDEV 6.0f


#define OUTLIER_REJECTION_STDDEV_MULT 2.5f
#define MIN_SAMPLE_QUALITY_WEIGHT 0.3f
#define OPTIMAL_SAMPLE_COUNT 10
#define MEDIAN_FILTER_SIZE 5


#define NO_SIGNAL_DECAY_RATE 0.9f
#define NO_SIGNAL_TIMEOUT_MS 3000
#define SIGNAL_LOSS_FAST_DECAY_RATE 0.7f


#define SIGMOID_MIDPOINT -60.0f
#define SIGMOID_STEEPNESS 0.3f


#define SLOPE_HISTORY_SIZE 3
#define SLOPE_VARIANCE_THRESHOLD 0.001f
...
#define LIGHT_SLEEP_DELAY_MS 20000
#define DEEP_SLEEP_DELAY_MS 30000
#define RFID_LIGHT_SLEEP_ON_MS 200
#define RFID_LIGHT_SLEEP_OFF_MS 500
#define RFID_DEEP_SLEEP_ON_MS 100
#define RFID_DEEP_SLEEP_OFF_MS 1000
#define BLE_DEEP_SLEEP_INTERVAL_MS 12000
#define BLE_DEEP_SLEEP_DURATION_MS 8000
...
struct RSSIAnalysis {
    ...
    float sampleQualityWeight = 1.0f;


    float historicalSuccessRate = 0.0f;
    int authAttempts = 0;
    int authSuccesses = 0;
    bool needsCalibrationAdjustment = false;
...
bool ghostKeyEnabled = true;
bool ghostPowerEnabled = true;
bool accessoryInputAuth = false;



bool accessoryInputPinHigh = false;
bool lastAccessoryInputPinHigh = false;
unsigned long accessoryPinLowTime = 0;
bool accessoryPinAutoLockActive = false;
bool accessoryPinFirstRead = true;


bool wasRfidAuthenticated = false;
bool unauthorizedBeepPlayed = false;
unsigned long lastUnauthorizedBeepTime = 0;


bool rfidHeldForConfig = false;
unsigned long rfidConfigHoldStartTime = 0;
unsigned long lastRfidReadTime = 0;
#define RFID_CONFIG_HOLD_TIME 10000
#define RFID_CONTINUOUS_READ_TIMEOUT 2000


bool rfidHeldForConfigExit = false;
unsigned long rfidConfigExitHoldStartTime = 0;
#define RFID_CONFIG_EXIT_HOLD_TIME 10000


PowerState currentPowerState = POWER_ACTIVE;
bool wasAuthenticated = false;
unsigned long lastAuthenticatedTime = 0;
unsigned long lastUnauthenticatedTime = 0;
unsigned long rfidLightSleepCycleStart = 0;
bool rfidLightSleepOn = true;
unsigned long rfidDeepSleepCycleStart = 0;
bool rfidDeepSleepOn = true;
unsigned long bleDeepSleepAdvertiseStart = 0;
bool bleDeepSleepAdvertising = false;
bool bleAdvertisingStopped = false;


unsigned long lightSleepTimeout = 0;
bool quickConfidenceCompleted = false;
#define QUICK_CONFIDENCE_LOW_THRESHOLD 35.0f
#define QUICK_CONFIDENCE_HIGH_THRESHOLD 45.0f
...
RTC_DATA_ATTR bool useHardwareDeepSleep = true;


bool brakeWakeUpInProgress = false;
unsigned long brakeWakeUpTime = 0;
bool brakeWakeUpActiveMode = false;
unsigned long brakeWakeUpActiveTime = 0;
#define BRAKE_WAKEUP_ACTIVE_TIME 30000
...
bool restartPendingForBluetooth = false;
...
byte tagData[5];
byte storedRfidKeys[MAX_RFID_KEYS][5];
String storedRfidNames[MAX_RFID_KEYS];
int numStoredKeys = 0;
bool rfidPairingMode = false;
bool rfidAuthenticated = false;
unsigned long rfidAuthStartTime = 0;
...
# 554 "/Users/zkhan/GhostKey/src/GhostKey.ino"
...
bool brakeButtonActionProcessed = false;
...
unsigned long calibrationDuration = 30000;
...
#line 655 "/Users/zkhan/GhostKey/src/GhostKey.ino"
void startWebSession() {
...
# 730 "/Users/zkhan/GhostKey/src/GhostKey.ino"
void cleanupNVSStorage() {
...
void startBLEAdvertising(bool discoverable) {
    ...
    if(discoverable) {
        ...
        adv_data.setCompleteServices(BLEUUID((uint16_t)0x1812));
...
bool decodeTag(unsigned char *buf)
{
  ...
  {
    timeCount = 0;
    while(0 == digitalRead(RFID_DEMOD_OUT))
    {
...
void setup() {
    ...
    if (preferences.getBool("bt_restart", false)) {
        ...
        preferences.putBool("bt_restart", false);
...
void loop() {
        ...
        if (ghostPowerEnabled) {
            ...
            if (accessoryPinFirstRead) {
                ...
                if (!accessoryInputPinHigh && lastAccessoryInputPinHigh) {
                    ...
                    accessoryPinAutoLockActive = false;
    ...
    static unsigned long currentRSSIInterval = RSSI_UPDATE_INTERVAL;
    ...
    if (currentState == CONFIG_MODE) {
        ...
        if (millis() - lastWebServerUpdate >= 50) {
    ...
    if(scanForTag(tagData) == true) {
        ...
        if (rfidPairingMode) {
            ...
            if ((!ghostKeyEnabled && ghostPowerEnabled && currentState != CONFIG_MODE) ||
                (!firstSetupComplete && currentState != CONFIG_MODE)) {
                if (!rfidHeldForConfig) {
                    ...
                    if (!ghostKeyEnabled && ghostPowerEnabled) {
                        ...
                        Serial.println("RFID: Config hold timer started (setup not complete)");
                    }
                } else {
    ...
    if (!firstRSSIScanDone && !rssiTimeoutChecked && firstRSSIScanStartTime > 0) {
        if (millis() - firstRSSIScanStartTime >= 10000) {
    ...
    if (!firstRfidReadDone && !rfidTimeoutChecked && firstRfidScanStartTime > 0) {
        if (millis() - firstRfidScanStartTime >= 15000) {
...
void setupPins() {
    ...
    pinMode(ACCESSORY_INPUT_PIN, INPUT);
...
void buzzerOn() {
    ledcWrite(BUZZER_PWM_CHANNEL, BUZZER_PWM_DUTY_CYCLE);
}
...
void handleButtonPress() {
    ...
    if ((millis() - buttonDebounceTime2) > DEBOUNCE_DELAY) {
        if (buttonReading == LOW && !buttonPressed) {
            ...
            buttonPressStartTime = millis();
            isLongPressDetected = false;
            brakeButtonActionProcessed = false;
...
void updateSystemState() {
    ...
    if (currentState == CONFIG_MODE) {
        ...
        if (millis() - lastConfigLedUpdate > 500) {
        ...
        if (millis() - lastEngineLedUpdate > 20) {
...
void exitConfigMode() {
    ...
    if (wifiEnabled) {
        ...
        const unsigned long GRACEFUL_SHUTDOWN_TIMEOUT = 200;
...
const char* getDevicesJson() {
    ...
    int remaining = sizeof(cachedDevicesJson) - 10;
    ...
    for (int i = 0; i < bondedCount && remaining > 150; i++) {
...
void setupWebServer() {
    ...
    server.on("/", HTTP_GET, [](){
        ...
        if (!firstSetupComplete) {
            ...
            bool recentSetupCompletion = (millis() - lastSetupCompletionTime < 5000);

            if (ESP.getFreeHeap() < 40000 && !recentSetupCompletion) {
...
# 5202 "/Users/zkhan/GhostKey/src/GhostKey.ino"
int8_t medianFilter(int8_t newValue) {
...
void addRSSIReading(int8_t rssi, esp_bd_addr_t address) {
    ...
    if (now - rssiAnalysis.lastAnalysis >= 250) {
        performRSSIAnalysis();
...
float getAdaptiveStabilityThreshold(float meanRSSI) {
    if (meanRSSI >= -50) return 15.0f;
    if (meanRSSI >= -65) return 12.0f;
    if (meanRSSI >= -75) return 8.0f;
    return 5.0f;
}
...
float calculateTrendScore() {
    ...
    for (int i = 0; i < RSSI_MEDIUM_TERM_SIZE; i++) {
        if (rssiAnalysis.mediumTerm[i].valid &&
            ...
            float x = (float)(now - rssiAnalysis.mediumTerm[i].timestamp);
    ...
    float trendStrength = abs(slope) * 1000.0f;
    float score = 0.0f;
...
void performRSSIAnalysis() {
    ...
    rssiAnalysis.shortTermStats = calculateBufferStatistics(
        rssiAnalysis.shortTerm, RSSI_SHORT_TERM_SIZE, 2000);

    rssiAnalysis.mediumTermStats = calculateBufferStatistics(
        rssiAnalysis.mediumTerm, RSSI_MEDIUM_TERM_SIZE, 10000);

    rssiAnalysis.longTermStats = calculateBufferStatistics(
        rssiAnalysis.longTerm, RSSI_LONG_TERM_SIZE, 60000);
...
void updatePowerManagement() {
    ...
    if (brakeWakeUpInProgress && currentPowerState == POWER_LIGHT_SLEEP) {
        if (millis() - brakeWakeUpTime >= 2000) {
        ...
        if (isBleConnected && hasConnectedDevice) {
            ...
            if (rssiAnalysis.shortTermStats.validSamples >= 3) {
                ...
                if (currentConfidence < 15.0f && !quickConfidenceCompleted &&
                    (currentTime - bleDeepSleepAdvertiseStart) < 3000) {
                    quickConfidenceCompleted = true;
            ...
            if (currentTime - lastPeakUpdate >= 1000) {
